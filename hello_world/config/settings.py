...
class Settings:
    ...
    def __init__(
        self, config_file: Optional[Union[str, Path]] = None, auto_reload: bool = True
    ):
        ...
        try:
            with self._lock:
                ...
                if "system_prompts" in config:
                    ...
                    self.system_prompts.default = prompts_config.get(
    ...
    def save_to_file(self, file_path: Optional[Union[str, Path]] = None) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to save settings to file", file=str(save_path), error=str(e)
            )
            raise
    ...
    def validate(self) -> list[str]:
        ...
        if self.timeouts.ai_response_timeout <= 0:
            issues.append(
                f"Invalid AI response timeout: {self.timeouts.ai_response_timeout}"
            )
        if self.timeouts.tts_generation_timeout <= 0:
            issues.append(
                f"Invalid TTS timeout: {self.timeouts.tts_generation_timeout}"
            )
