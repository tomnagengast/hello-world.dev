    ...
    def write(self, audio_data: np.ndarray) -> int:
        ...
        with self._lock:
            ...
            if self.write_pos + write_len <= self.size:
                self.data[self.write_pos : self.write_pos + write_len] = audio_data[
                    :write_len
                ]
            else:
        ...
        with self._lock:
            ...
            if self.read_pos + read_len <= self.size:
                ...
                result = np.concatenate(
                    [self.data[self.read_pos :], self.data[: read_len - first_part]]
                )
...
class WhisperKitProvider(STTProvider):
    ...
    def audio_callback(
        ...
        try:
            ...
            if written < len(audio_data):
                logger.warning(
                    "Ring buffer overflow", requested=len(audio_data), written=written
                )
    ...
    def initialize(self) -> None:
        ...
        logger.info(
            "Initializing WhisperKit provider with sounddevice",
            model=self.model,
                ...
                try:
                    ...
                    if len(chunk_buffer) >= chunk_duration_samples:
                        ...
                        chunk_array = np.array(
                            chunk_buffer[:chunk_duration_samples], dtype=np.float32
                        )
                        ...
                        if self.process and self.process.stdin:
                            try:
                                ...
                                logger.error(
                                    "Failed to send audio to WhisperKit", error=str(e)
                                )
                                break
    ...
    def stream_transcripts(self) -> Iterator[Transcript]:
        ...
        while self.is_running:
            try:
                ...
                latency_ms = (
                    (processing_time - self.last_audio_time) * 1000
