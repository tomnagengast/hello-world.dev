...
class WhisperKitFileProvider(STTProvider):
    ...
    def initialize(self) -> None:
        ...
        logger.info("Initializing WhisperKit file provider", 
                   model=self.model,
    ...
    def process_file(self, file_path: Union[str, Path]) -> Iterator[Transcript]:
        ...
        try:
            ...
            if return_code != 0:
                ...
                logger.error("WhisperKit process failed", 
                           return_code=return_code,
                           stderr=stderr_output)
                raise RuntimeError(f"WhisperKit failed with code {return_code}: {stderr_output}")
            ...
            logger.error("Error processing file", error=str(e), file_path=str(file_path))
            raise
        finally:
            if self.process:
                try:
