...
class ProviderRegistry:
    ...
    def register_stt_provider(self, name: str, provider_class: Type[STTProvider], 
        ...
        logger.info("Registered STT provider", name=name, class_name=provider_class.__name__)
        
    def register_ai_provider(self, name: str, provider_class: Type[AIProvider],
                            config_getter: Callable[[], Dict[str, Any]] = None) -> None:
        """Register an AI provider."""
        self._ai_providers[name] = provider_class
        if config_getter:
            self._provider_configs[f"ai:{name}"] = config_getter
        logger.info("Registered AI provider", name=name, class_name=provider_class.__name__)
        
    def register_tts_provider(self, name: str, provider_class: Type[TTSProvider],
                             config_getter: Callable[[], Dict[str, Any]] = None) -> None:
        """Register a TTS provider."""
        self._tts_providers[name] = provider_class
        if config_getter:
            self._provider_configs[f"tts:{name}"] = config_getter
        logger.info("Registered TTS provider", name=name, class_name=provider_class.__name__)
