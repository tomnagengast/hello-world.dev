...
class InterruptionHandler:
    ...
    def _update_dynamic_threshold(self) -> None:
        ...
        recent_levels.sort()
        self.noise_floor = np.mean(recent_levels[:len(recent_levels)//4])  # Bottom 25%
        
        # Set dynamic threshold above noise floor
        self.dynamic_threshold = max(self.noise_floor * 3.0, 0.01)
        
        logger.debug("Updated audio thresholds",
    ...
    def get_voice_activity_stats(self) -> dict:
        ...
        return {
            ...
            "silence_duration_ms": (time.time() - self.last_voice_time) * 1000 if self.last_voice_time else 0,
