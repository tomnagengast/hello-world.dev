...
class TestAICLI:
    ...
    def test_basic_text_input(self, mock_claude_provider):
        ...
        mock_claude_provider.stream_response.return_value = [
            ...
            AIResponse(
                text="", is_first=False, is_final=True, full_text="Hello there!"
            ),
        ]

        with patch(
            "hello_world.cli.ai.ClaudeProvider", return_value=mock_claude_provider
        ):
    ...
    def test_gemini_provider(self, mock_gemini_provider):
        ...
        mock_gemini_provider.stream_response.return_value = [
            AIResponse(
                ...
                is_first=True,
                is_final=True,
                full_text="Gemini response",
            )
        ]

        with patch(
            "hello_world.cli.ai.GeminiProvider", return_value=mock_gemini_provider
        ):
    ...
    def test_stdin_input(self, mock_claude_provider):
        ...
        mock_claude_provider.stream_response.return_value = [
            AIResponse(
                text="Response from stdin",
                is_first=True,
                is_final=True,
                full_text="Response from stdin",
            )
        ]
    ...
    def test_conversation_context(self, mock_claude_provider, tmp_path):
        ...
        with patch(
            ...
            result = self.runner.invoke(
                ai, ["--input", "Continue", "--context", str(context_file)]
            )
    ...
    def test_custom_system_prompt(self, mock_claude_provider):
        ...
        mock_claude_provider.stream_response.return_value = [
            AIResponse(
                ...
                is_first=True,
                is_final=True,
                full_text="Custom system response",
            )
        ]
        ...
        with patch("hello_world.cli.ai.ClaudeProvider") as mock_class:
            ...
            result = self.runner.invoke(
                ai, ["--input", "test", "--system", "You are a specialized assistant."]
            )

        assert result.exit_code == 0
        # Verify system prompt was passed to provider
        mock_class.assert_called_once_with(
            system_prompt="You are a specialized assistant.", streaming=True
        )
    ...
    def test_no_streaming(self, mock_claude_provider):
        ...
        mock_class.assert_called_once_with(
            system_prompt="You are a helpful AI assistant.", streaming=False
        )
    ...
    def test_invalid_context_file(self):
        ...
        result = self.runner.invoke(
            ai, ["--input", "test", "--context", "nonexistent.json"]
        )
    ...
    def test_model_parameter(self, mock_gemini_provider):
        ...
        mock_gemini_provider.stream_response.return_value = [
            AIResponse(
                text="Model test", is_first=True, is_final=True, full_text="Model test"
            )
        ]
