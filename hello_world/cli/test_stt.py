...
def read_audio_from_stdin() -> Path:
    ...
    try:
        ...
        logger.info("Audio data read from stdin", temp_file=str(temp_path), size=len(audio_data))
        return temp_path
...
def cleanup_temp_file(file_path: Path) -> None:
    ...
    try:
        ...
        logger.warning("Failed to cleanup temporary file", file_path=str(file_path), error=str(e))
...
@click.option(
    "--provider",
    default="whisperkit",
    help="STT provider to use (default: whisperkit)"
)
@click.option(
    "--model",
...
@click.option(
    "--list-providers",
    is_flag=True,
    help="List available STT providers and exit"
)
def stt(
    ...
    if list_providers:
        ...
        providers = ["whisperkit"]  # For now, we only support whisperkit file processing
        for p in providers:
            click.echo(f"  - {p}")
        return
