...
@click.option(
    "--input", "-i", 
    help="Text input for the AI (if not provided, reads from stdin)"
)
@click.option(
    "--provider", "-p",
    default="claude",
...
@click.option(
    "--json", "json_output",
    is_flag=True,
    help="Output response as JSON with metadata"
)
@click.option(
    "--metrics",
    is_flag=True,
...
def ai(
    ...
    if context:
        try:
            with open(context, 'r') as f:
                ...
                logger.info("Loaded conversation context", messages=len(conversation_history))
    ...
    try:
        ...
        first_token_latency_ms = (first_token_time - start_time) * 1000 if first_token_time else total_latency_ms
        ...
        if json_output:
            ...
            if metrics:
                ...
                click.echo(f"First token latency: {first_token_latency_ms:.2f}ms", err=True)
